...
export const ProductItem = ({ product }) => {
  ...
  // Manejar cambio en el input (sin límite de stock)
  const handleCantidadChange = (e) => {
    let newValue = parseInt(e.target.value);
    if (isNaN(newValue)) newValue = 1;
    if (newValue < 1) newValue = 1;
    setCantidad(newValue);   // Permite cualquier número, incluso mayor al stock
  };
  ...
  const handleAgregar = async () => {
    ...
    if (cantidad > product.stock) {
      ...
      setCantidad(product.stock);   // Ajusta la cantidad al máximo (feedback visual)
      return; // ✅ No agrega al carrito
    }
    setIsAdding(true);
    try {
      await dispatch(
        agregarAlCarrito({ id_producto: product.id_producto, cantidad })
      ).unwrap();
